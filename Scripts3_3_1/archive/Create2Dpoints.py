...
with arcpy.da.SearchCursor(wwpt_merge, ['OID@', wwpt_wellid_field, wwpt_etid_field, wwpt_label_field,
                                        'NEAR_DIST', 'SHAPE@X', 'SHAPE@Y', 'OnLine_DIST','elevation']) as wwpt:
    for well in wwpt:
        index = well[0]
        wellid = well[1]
        et_id = well[2]
        ...
        with arcpy.da.InsertCursor(pointfile, ['SHAPE@', wwpt_wellid_field, wwpt_etid_field, wwpt_label_field, 'x_coord',
                                               'y_coord', 'x_coord_2d','y_coord_2d','distance', 'elevation', 'pct_dist','OnLine_DIST']) as cursor:
            # Create geometry and fill in field values, saving true coordinates in attribute
            cursor.insertRow([point_geometry, wellid, et_id, label, real_x, real_y, x_coord_2d, y_coord_2d, dist, real_z, pct_dist, x_coord_meters])
