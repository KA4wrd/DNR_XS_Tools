...
# Create 2D points (Traditional display, verified well locations only)
...
'''
This script creates point files that are used to visualize well feature locations
...
fields_2d_point = [['elevation', 'FLOAT'],['Data_Source','TEXT']]
...
with arcpy.da.SearchCursor(wwpt_merge, ['OID@', wwpt_wellid_field, wwpt_etid_field, wwpt_label_field,
                                        'NEAR_DIST', 'SHAPE@X', 'SHAPE@Y', 'OnLine_DIST','elevation','Data_Source']) as wwpt:
    for well in wwpt:
        index = well[0]
        wellid = well[1]
        et_id = well[2]
        ...
        with arcpy.da.InsertCursor(pointfile, ['SHAPE@', wwpt_wellid_field, wwpt_etid_field, wwpt_label_field, 'x_coord',
                                               'y_coord', 'x_coord_2d','y_coord_2d','distance', 'elevation', 'pct_dist','Data_Source']) as cursor:
...
pointfile_copy = os.path.join(workspace, "wwpt_2d_xsecview_prj")
