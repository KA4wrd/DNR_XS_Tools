...
# Create Unloc 2D Pump Level points (Traditional display, unverified location wells only)
...
'''
This script creates point files that are used to visualize well pump level locations
...
FieldExists(wwpt_file_orig, "dropp_len")
...
fields_base = [[wwpt_wellid_field, well_id_field_type], [xsln_etid_field, 'TEXT', '', 3],
               ['x_coord', 'DOUBLE'], ['y_coord', 'DOUBLE'], ['x_coord_2d', 'DOUBLE'], ['y_coord_2d', 'DOUBLE'], ['dropp_len', 'DOUBLE'], ['dpl_elev', 'DOUBLE']]
...
arcpy.management.CreateFeatureclass(workspace, "dpl_unloc_2d_xsecview", "POINT", '',
                                    'DISABLED', 'DISABLED')

#set point shapefile filepath variable
pointfile = os.path.join(workspace, "dpl_unloc_2d_xsecview")
...
wwpt_fields = [["NEAR_FID", "LONG"], ["NEAR_DIST", "DOUBLE"], ["NEAR_X", "DOUBLE"],
               ["NEAR_Y", "DOUBLE"], ["OnLine_DIST", "FLOAT"], ["dpl_elev", "DOUBLE"]]
...
with arcpy.da.SearchCursor(wwpt_merge, ['OID@', wwpt_wellid_field, wwpt_etid_field,
                                        'NEAR_DIST', 'SHAPE@X', 'SHAPE@Y', 'OnLine_DIST','dem','dropp_len','dpl_elev','Data_Source']) as wwpt:
    for well in wwpt:
        index = well[0]
        wellid = well[1]
        et_id = well[2]
        ...
        with arcpy.da.InsertCursor(pointfile, ['SHAPE@', wwpt_wellid_field, wwpt_etid_field,'x_coord',
                                               'y_coord', 'x_coord_2d','y_coord_2d','distance', 'dem', 'pct_dist', 'dropp_len','dpl_elev','Data_Source']) as cursor:
            # Create geometry and fill in field values, saving true coordinates in attribute
            cursor.insertRow([point_geometry, wellid, et_id, real_x, real_y, x_coord_2d, y_coord_2d, dist, well_z, pct_dist, dpl_meas, dpl_z, data_source])
...
pointfile_copy = os.path.join(workspace, "dpl_unloc_2d_xsecview_prj")
