...
# Create Unloc 2D points (Traditional display, unverified well locations only)
...
'''
This script creates point files that are used to visualize unloc well feature locations
along defined cross sections at ground elevation. Outputs are: a point file in 2D cross-section space
to use for cross-section creation and editing. Data is retrieved from a well
...
if (len(sys.argv) > 1):
    ...
    wwpt_label_field = arcpy.GetParameterAsText(8) #well label field in well point file - must be TEXT type
    buffer_dist = int(arcpy.GetParameterAsText(6))
    vertical_exaggeration = int(arcpy.GetParameterAsText(7))
    spatialref_2d = arcpy.GetParameterAsText(9) # .prj file for custom coordinate system for 2D display
    ...
    wwpt_label_field = 'well_label' #well label field in well point file - must be TEXT type
    buffer_dist = 500
...
FieldExists(wwpt_file_orig, "WELL_LABEL")
...
with arcpy.da.SearchCursor(wwpt_file_orig, [wwpt_wellid_field, wwpt_etid_field, wwpt_label_field]) as wwpt_records:
    for row in wwpt_records:
        ...
        if label not in wwpt_label_list:
            wwpt_label_list.append(label)
...
fields_base = [[wwpt_wellid_field, well_id_field_type], [xsln_etid_field, 'TEXT', '', 3], [wwpt_label_field, 'TEXT', '', 10],
               ['x_coord', 'DOUBLE'], ['y_coord', 'DOUBLE'], ['x_coord_2d', 'DOUBLE'], ['y_coord_2d', 'DOUBLE'] ]
...
fields_2d_point = [['dem', 'FLOAT'],['Data_Source','TEXT']]
...
arcpy.management.CreateFeatureclass(workspace, "wwpt_unloc_2d_xsecview", "POINT", '',
                                    'DISABLED', 'DISABLED')

#set point shapefile filepath variable
pointfile = os.path.join(workspace, "wwpt_unloc_2d_xsecview")
...
with arcpy.da.SearchCursor(wwpt_merge, ['OID@', wwpt_wellid_field, wwpt_etid_field, wwpt_label_field,
                                        'NEAR_DIST', 'SHAPE@X', 'SHAPE@Y', 'OnLine_DIST','dem','Data_Source']) as wwpt:
    for well in wwpt:
        index = well[0]
        wellid = well[1]
        et_id = well[2]
        ...
        dist = well[4] #NEAR_DIST field
        pct_dist = dist / buffer_dist * 200 #percent distance
        real_x = well[5] #true x coordinate of well
        real_y = well[6] #true y coordinate of well
        real_z = well[8]
        if real_z == None:
            printit("Error: Well number {0} is null in elevation field. Skipping.".format(wellid))
            continue
        x_coord_meters = well[7]
        ...
        with arcpy.da.InsertCursor(pointfile, ['SHAPE@', wwpt_wellid_field, wwpt_etid_field, wwpt_label_field, 'x_coord',
                                               'y_coord', 'x_coord_2d','y_coord_2d','distance', 'dem', 'pct_dist','Data_Source']) as cursor:
            # Create geometry and fill in field values, saving true coordinates in attribute
            cursor.insertRow([point_geometry, wellid, et_id, label, real_x, real_y, x_coord_2d, y_coord_2d, dist, real_z, pct_dist, data_source])

endtime = datetime.datetime.now()
elapsed = endtime - starttime
printit('2D point geometry completed at {0}. Elapsed time: {1}'.format(endtime, elapsed))

#add new labeling fields to get around well_label character restriction
labeling_fields = [['well_label_2d', 'TEXT'],['Notes', 'TEXT']]
pts_2d_table_select_1 = arcpy.management.AddFields(pointfile, labeling_fields)

# # make copy of unprojected 2D pt file
pointfile_copy = os.path.join(workspace, "wwpt_unloc_2d_xsecview_prj")
arcpy.management.CopyFeatures(pointfile, pointfile_copy)
...
printit('Create 2D Points tool completed at {0}. Elapsed time: {1}. You did it!'.format(toolend, toolelapsed))
